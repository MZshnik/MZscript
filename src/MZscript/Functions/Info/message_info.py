...
class MessageInfo(FunctionsHandler):
    def __init__(self, handler):
        super().__init__()
        self.handler = handler
        self.bot = handler.client.bot

    async def func_messageinfo(self, ctx: disnake.Message, args: str):
        """
        `$messageInfo[(channel;message);param]`
        ### Example:
        `$messageInfo[author]`
        """
        args_list = await self.get_args(await self.is_have_functions(args, ctx))
        if len(args_list) > 3 or len(args_list) == 0:
            raise ValueError("$messageInfo: Too many or no args provided")
        ...
        if args_list[0].isdigit() and len(args_list) > 1:
            try:
                ...
                raise SyntaxError(f"$messageInfo: Cannot find channel \"{args_list[0]}\"")
        else:
            args_list.insert(0, channel)
        ...
        if args_list[1].isdigit() and len(args_list) > 2:
            try:
                ...
                raise SyntaxError(f"$messageInfo: Cannot find message \"{args_list[1]}\"")
        else:
            args_list.insert(1, message)
